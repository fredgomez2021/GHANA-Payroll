CREATE PROCEDURE [dbo].[PayRep1_Month]
   @PeriodMonth AS VARCHAR(20),
   @PeriodYear AS VARCHAR(4)
AS
...
insert  into xPayRep1
   select @PeriodMonth + '/1/' + @PeriodYear AS period1, p.empLoc AS [emp_loc],p.employeeName AS [Employee_Name],p.project AS [primary_task_id],
   isnull(p.positionCode,'') as job_position_code,
   isnull(c.hierarchy,'') as hierarchy,'' as employee_pin,
   ...
   sum(isnull(m13thmo,0)) as m13thmo,sum(isnull(e.ytd_13thmo,0)) as ytd_13thmo,
   ...
   isnull(pagibig_er,0)+isnull(m13thmo,0)+(isnull(other_earn4,0)+isnull(other_earn5,0))) as tpay_exp,
   sum(isnull((reg_hrs2+ot_hrs2),0)) as hrsWorked
      ...
      where MONTH(period1)=12
		and YEAR(period1)=2016
	  group by p.empLoc, p.employeeName, p.project, p.positionCode, c.hierarchy,
		p.atmNumber, e.watm
...
insert into dbo.xPayRep2
   ...
   sum(isnull(hrsWorked,0)) as hrsWorked
      from dbo.xPayRep1 p
      where MONTH(period1)=@PeriodMonth
		and YEAR(period1)=@PeriodYear
