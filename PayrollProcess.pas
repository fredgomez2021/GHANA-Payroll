...
type
  TfrmPayrollProcess = class(TForm)
    ...
    function GetAnnualSalary(const AEmpCode: string; const AFromDate: TDate): Double;
    function GetYearToMonthBonus(const AEmpCode: string;const CurrentCutoffBonus: Double): Double;

    function GetYearToMonthSalary(const AEmpCode: string; const AFromDate: TDate): Double;

    procedure CalculateBonusTax(const AEmpPin: Integer; const ABasicSalary: Currency; const ABonusAmount: Currency; out AdditionalTaxableIncome: Currency; out BonusTax: Currency);
...
var
  ...
  vBonusOthers, vBonusTraining, vBonusFreeField1 : Currency;
  ...
  vTaxableIncome : Currency;
  vTotalAnnualSalary, vTotalBonus, vBonusThreshold, vExcessBonusToAddToTax : Currency;
  vBonusTax : Currency;
  vTotalYearToMonthSalary : Currency;

  // enhanced other earnings/deductions
  vOther_EarnAfterTax1, vOther_EarnAfterTax2 : Currency;
  vOther_EarnAfterTax_Desc1, vOther_EarnAfterTax_Desc2 : String;
  vOther_DedAfterTax1, vOther_DedAfterTax2, vOther_DedAfterTax3, vOther_DedAfterTax4 : Currency;
  vOther_DedAfterTax5, vOther_DedAfterTax6, vOther_DedAfterTax7 : Currency;
  vOther_DedAfterTax_Desc1, vOther_DedAfterTax_Desc2, vOther_DedAfterTax_Desc3, vOther_DedAfterTax_Desc4 : String;
  vOther_DedAfterTax_Desc5, vOther_DedAfterTax_Desc6, vOther_DedAfterTax_Desc7 : String;

  vMPF_ER, vMPF_EE, vPrev_MPF_ER, vPrev_MPF_EE : Currency;
...
begin
  ...
  if(day<>20) then
  begin
    ShowMessage('The Period To should be 20th of the month.');
  ...
  strSt := 'UPDATE dtaTimeSecurity SET tStatus = 1, tFrom = ' + sPeriod1 + ', tTo = ' + sPeriod2;

  ADOConnection1.Execute(strSt);

  // check if the payroll period has been locked for processing //
  ADODataSet1.CommandText := 'SELECT * FROM dtaLockPayPeriod WHERE Period1='+sPeriod2;
  ADODataSet1.Open;
  ADODataSet1.First;
  ...
  ADODataSet1.CommandText := 'SELECT * FROM dtaPayrollProcess WHERE Period1='+sPeriod2;
  ADODataSet1.Open;
  ADODataSet1.First;
  I := ADODataSet1.RecordCount;
  ADODataSet1.Close;
  ...
  if (chkContinueProcess.Checked = FALSE) then
  begin
    ADOConnection1.Execute('DELETE FROM dtaPayrollProcess WHERE Period1=' + sPeriod2);
  end;
...
begin
  ...
  if(DayOfTheMonth(DateTimePicker1.Date))= 21 then
    if myMonth = 12 then
      dt := EncodeDate( myYear + 1, 1, 20)
    else
...
begin
  ...
  //connESS.ConnectionString := CommonModule.ReadInitConn_ESS(CommonModule.ToConnect('Null'));
  //connESS.Connected := TRUE;
...
begin
  ...
  if (chkContinueProcess.Checked) then
  begin
    ADODataSet1.CommandText := 'SELECT * ' +
      'FROM dtaEmployees (nolock) ' +
      'WHERE Emp_status = ''ACTIVE'' '+
  //  'AND (Employee_PIN = 714 OR employee_pIN = 414) '+
  //    'AND employee_pin = 607 ' +
      'AND ProcessIncluded = 1 ' +
      'AND Employee_PIN NOT IN (SELECT Employee_PIN FROM dtaPayrollProcess WHERE Period1=' + sPeriod1 + ') ' +
  ...
  begin
    ADODataSet1.CommandText := 'SELECT * FROM dtaEmployees (nolock) WHERE Emp_status = ''ACTIVE'' '+
      'AND ProcessIncluded = 1 ' +
//      'AND employee_pin = 2330200 ' +
//      'AND employee_pin = 2330119 ' +
      'ORDER BY EMPLOYEE_NAME';
  end;
  ...
  begin
    ...
    repeat
      ...
      vOther_EarnAfterTax1 := 0;
      vOther_EarnAfterTax2 := 0;
      vOther_EarnAfterTax_Desc1 := '';
      vOther_EarnAfterTax_Desc2 := '';
      vOther_DedAfterTax1 := 0;
      vOther_DedAfterTax2 := 0;
      vOther_DedAfterTax3 := 0;
      vOther_DedAfterTax4 := 0;
      vOther_DedAfterTax5 := 0;
      vOther_DedAfterTax6 := 0;
      vOther_DedAfterTax7 := 0;

      vOther_DedAfterTax_Desc1 := '';
      vOther_DedAfterTax_Desc2 := '';
      vOther_DedAfterTax_Desc3 := '';
      vOther_DedAfterTax_Desc4 := '';
      vOther_DedAfterTax_Desc5 := '';
      vOther_DedAfterTax_Desc6 := '';
      vOther_DedAfterTax_Desc7 := '';
      ...
      begin
        ...
        begin
          ...
          // earnings after tax
          // amount
          if Not ADODataSet2.FieldByName('Other_Earn_Amount1_AfterTax').IsNull  then
              vOther_EarnAfterTax1 := ADODataSet2.FieldByName('Other_Earn_Amount1_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Earn_Amount2_AfterTax').IsNull  then
              vOther_EarnAfterTax2 := ADODataSet2.FieldByName('Other_Earn_Amount2_AfterTax').Value;

          // description
          if Not ADODataSet2.FieldByName('Other_Earn_Description1_AfterTax').IsNull  then
              vOther_EarnAfterTax_Desc1 := ADODataSet2.FieldByName('Other_Earn_Description1_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Earn_Description2_AfterTax').IsNull  then
              vOther_EarnAfterTax_Desc2 := ADODataSet2.FieldByName('Other_Earn_Description2_AfterTax').Value;
          ...
          // amount
          if Not ADODataSet2.FieldByName('Other_Ded_Amount1_AfterTax').IsNull  then
              vOther_DedAfterTax1 := ADODataSet2.FieldByName('Other_Ded_Amount1_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount2_AfterTax').IsNull  then
              vOther_DedAfterTax2 := ADODataSet2.FieldByName('Other_Ded_Amount2_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount3_AfterTax').IsNull  then
              vOther_DedAfterTax3 := ADODataSet2.FieldByName('Other_Ded_Amount3_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount4_AfterTax').IsNull  then
              vOther_DedAfterTax4 := ADODataSet2.FieldByName('Other_Ded_Amount4_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount5_AfterTax').IsNull  then
              vOther_DedAfterTax5 := ADODataSet2.FieldByName('Other_Ded_Amount5_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount6_AfterTax').IsNull  then
              vOther_DedAfterTax6 := ADODataSet2.FieldByName('Other_Ded_Amount6_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Amount7_AfterTax').IsNull  then
              vOther_DedAfterTax7 := ADODataSet2.FieldByName('Other_Ded_Amount7_AfterTax').Value;

          // description
          if Not ADODataSet2.FieldByName('Other_Ded_Description1_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc1 := ADODataSet2.FieldByName('Other_Ded_Description1_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description2_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc2 := ADODataSet2.FieldByName('Other_Ded_Description2_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description3_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc3 := ADODataSet2.FieldByName('Other_Ded_Description3_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description4_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc4 := ADODataSet2.FieldByName('Other_Ded_Description4_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description5_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc5 := ADODataSet2.FieldByName('Other_Ded_Description5_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description6_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc6 := ADODataSet2.FieldByName('Other_Ded_Description6_AfterTax').Value;
          if Not ADODataSet2.FieldByName('Other_Ded_Description7_AfterTax').IsNull  then
              vOther_DedAfterTax_Desc7 := ADODataSet2.FieldByName('Other_Ded_Description7_AfterTax').Value;
        end;
        ADODataSet2.Close;
                ...
                if not ADODataSet1.FieldByName('Date_Hired').IsNull then
                  ...
                  begin
                    ...
                    begin
                        ...
                        begin
                            vHoliday_Pay1 := vHoliday_Pay1 + ((ADODataSet1.FieldByName('Monthly_rate').Value / 21.75) * (vDays * ADODataSet2.FieldByName('Holiday_Rate').Value));
                            vRate_Day1 := vRate_Month1 / 21.75;
                        end
                        else
        ...
        begin
          ...
          ADODataSet2.CommandText := 'SELECT ISNULL(SUM(Days_Work),0) AS tDays_Work,' +
            ...
            'AND Actual_Out NOT IN (SELECT Actual_Out FROM dtaTime_Summary ' +
              'WHERE Employee_PIN = ' + ADODataSet1.FieldByName('Employee_PIN').AsString + ' ' +
              'AND Actual_Out IS NOT NULL ' +
              ' AND Work_Date BETWEEN DATEADD(d,-5,' + sPeriod1 + ') AND DATEADD(d,-1,' + sPeriod1 + ') ) ';
           ...
           begin
               ...
               begin
                    SQL := 'UPDATE dtaTime_Detail SET ' +
                      ...
                      ', Night_Diff_Hours_Hol = Night_Diff_Hours ' +
                      ' WHERE Time_ID = '+ wID;
        ...
        vBasic_Pay1 := vRate_Month1;

        //vBasic_Pay2 := vRate_Month1;
        ...
        begin
          vBasic_Pay2 := vRate_Hr2 * vReg_Hrs2;
        end; }
        ...
        // add 4th other earnings before tax
        vGross_Pay := vGross_Pay + vOther_Earn4;

        vGross_Pay := vGross_Pay + ( vOther_Ded1 + vOther_Ded2 + vOther_Ded3 );

        // deduct from basic pay to get net basic and get sss ee/er and tier 1 ee/er
        vNetBasic := vBasic_Pay1 + vOther_Earn1;
        ...
        SQL := 'SELECT ISNULL(Bonus_UPH,0) AS Bonus_UPH, ISNULL(Bonus_Quality,0) AS Bonus_Quality, ' +
          ...
          'ISNULL(Bonus_Others,0) AS Bonus_Others, ISNULL(Bonus_Training,0) AS Bonus_Training, ISNULL(Bonus_FreeField1,0) AS Bonus_FreeField1 ';
        SQL := SQL + 'FROM dtaBonusesDetails ';
        SQL := SQL + 'WHERE Period1 = ' + sPeriod2 + ' ';
        ...
        begin
          ...
          vBonusTraining := dsBonusesDetails.FieldByName('Bonus_Training').Value;
          vBonusFreeField1 := dsBonusesDetails.FieldByName('Bonus_FreeField1').Value;
        ...
        vGross_Pay := vGross_Pay + (vBonusUPH + vBonusQuality + vBonusRetention +
          vBonusAttendance + vBonusPAttendance + vBonusOthers + vBonusTraining + vBonusFreeField1);
        ...
        // this will be SSS tier - ghana
        if not ADODataSet1.FieldByName('MP2Share').IsNull then
        begin
          vMP2EmployeeShare := ADODataSet1.FieldByName('MP2Share').Value;
        end;

        if not ADODataSet1.FieldByName('MP2Share_ER').IsNull then
        begin
          vMP2EmployerShare := ADODataSet1.FieldByName('MP2Share_ER').Value;
        end;
        ...
        begin
          ...
          begin
            ...
            // Added Mandatory Provident Fund - Fred: 01/24/2021
            if Not ADODataSet2.FieldByName('MPF_EE').IsNull then
              vPrev_MPF_EE := ADODataSet2.FieldByName('MPF_EE').Value;
            if Not ADODataSet2.FieldByName('MPF_ER').IsNull then
              vPrev_MPF_ER := ADODataSet2.FieldByName('MPF_ER').Value;
          end;
          ADODataSet2.Close;
        end;  }
        ...
        begin
          vSSS_EE := vNetBasic * 0.055;
          vSSS_ER := vNetBasic * 0.13;
        end;
        ...
        begin
          ...
          begin
            ...
            vMPF_EE := ADODataSet2.FieldByName('MPF_EE').Value - vPrev_MPF_EE;
            vMPF_ER := ADODataSet2.FieldByName('MPF_ER').Value - vPrev_MPF_ER;
          end;
          ADODataSet2.Close;
        end; }
        ...
        begin
          ...
          if( vMC <= 1500 ) then
            ...
            begin
              vPagibig_EE := 10000 * 0.02;
              vPagibig_ER := 10000 * 0.02;
            end;
        ...
        //vTotalAnnualSalary := 0;
        vBonusThreshold := 0;
        vTotalBonus := 0;
        vExcessBonusToAddToTax := 0;
        vBonusTax := 0;

        vTotalYearToMonthSalary := 0;

        {// get total annual salary
        vTotalAnnualSalary := GetAnnualSalary(ADODataSet1.FieldByName('Employee_PIN').AsString, DateTimePicker1.Date);
        // get threshold
        vBonusThreshold := vTotalAnnualSalary * 0.15;
        // get total bonus
        vTotalBonus := GetAnnualBonus(ADODataSet1.FieldByName('Employee_PIN').AsString,
          (vBonusUPH + vBonusQuality + vBonusRetention + vBonusOthers)); }

        {// get total year to month salary
        vTotalYearToMonthSalary := GetYearToMonthSalary(ADODataSet1.FieldByName('Employee_PIN').AsString, DateTimePicker1.Date);
        // get threshold
        vBonusThreshold := vTotalYearToMonthSalary * 0.15;
        // get total bonus
        vTotalBonus := GetYearToMonthBonus(ADODataSet1.FieldByName('Employee_PIN').AsString,
          (vBonusUPH + vBonusQuality + vBonusRetention + vBonusOthers));

        // check if total bonus is greater than the threshold
        // if it is greater, get the difference and add that to regular tax
        // if it is less than the threshold, get 5% of it as bonus tax
        if vTotalBonus > vBonusThreshold then
        begin
          vExcessBonusToAddToTax := vTotalBonus - vBonusThreshold;
          vBonusTax := 0;
        end
        else
        begin
          vBonusTax := vTotalBonus * 0.05;
        end; }


        vTotalBonus := (vBonusUPH + vBonusQuality + vBonusRetention +
          vBonusAttendance + vBonusPAttendance + vBonusOthers + vBonusTraining + vBonusFreeField1);

        CalculateBonusTax(
          StrToInt(ADODataSet1.FieldByName('Employee_PIN').AsString),          // Employee PIN
          vBasic_Pay1,    // Basic Salary
          vTotalBonus,    // Bonus Amount
          vExcessBonusToAddToTax,
          vBonusTax
        );
        ...
        vTaxableIncome := 0;
        vTaxableIncome := (vGross_Pay - vTotalBonus) + vExcessBonusToAddToTax;

        // get employee and employee share
        // from gross basic
        vMP2EmployeeShare := vNetBasic * (vMP2EmployeeShare / 100);
        vMP2EmployerShare := vNetBasic * (vMP2EmployerShare / 100);
        
        // deduct sss and tier 1 from total gross before calculating tax
        vTaxableIncome := vTaxableIncome - (vSSS_EE + vMP2EmployeeShare);
        ...
        //begin
          ...
          begin
            ...
            begin
              // process the extracted production of the particular employee
              ADODataSet2.First;
              if vTaxableIncome >= ADODataSet2.FieldByName('Col7').Value then vCol:='Col7'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col6').Value then vCol:='Col6'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col5').Value then vCol:='Col5'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col4').Value then vCol:='Col4'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col3').Value then vCol:='Col3'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col2').Value then vCol:='Col2'
              else if vTaxableIncome >= ADODataSet2.FieldByName('Col1').Value then vCol:='Col1';
            end;
            if vCol <> 'Col0' then
            begin
              vExcess := vTaxableIncome - ADODataSet2.FieldByName(vCol).Value;
              vExcess_Per := 0;
              ADODataSet2.Close;
        ...
        // add bonus tax (if less than the threshold
        vWith_Tax := vWith_Tax + vBonusTax;

        // revised formula
        vTotal_Ded := vWith_Tax + vSSS_EE + vPhilhealth_EE + vPagibig_EE + vMPF_EE;

        vTotal_Ded := vTotal_Ded + ( vOther_DedAfterTax1 + vOther_DedAfterTax2 + vOther_DedAfterTax3 +
          vOther_DedAfterTax4 + vOther_DedAfterTax5 + vOther_DedAfterTax6 + vOther_DedAfterTax7 );
        ...
        // temp
        //vTotal_Ded := vTotal_Ded + vMP2EmployeeShare + vOther_EarnAfterTax1;
        //vNet_Pay := (vGross_Pay + vOther_EarnAfterTax2) - vTotal_Ded;
        ...
        // get net pay
        vNet_Pay := (vGross_Pay + vOther_EarnAfterTax1 + vOther_EarnAfterTax2) - vTotal_Ded;
        ...
        sAtmNumber := ADODataSet1.FieldByName('atmcardnumber').AsString;
        ...
        SQL := 'INSERT INTO dtaPayrollProcess ' +
            ...
            ' Bonus_Others, Bonus_Training, Bonus_FreeField1, Remarks, MP2_EE, MP2_ER, ' +
            ' Other_Earn1_AfterTax, Other_Earn2_AfterTax, ' +
            ' Other_Earn_Desc1_AfterTax, Other_Earn_Desc2_AfterTax, ' +
            ' Other_Ded1_AfterTax, Other_Ded2_AfterTax, Other_Ded3_AfterTax, ' +
            ' Other_Ded4_AfterTax, Other_Ded5_AfterTax, Other_Ded6_AfterTax, Other_Ded7_AfterTax, ' +
            ' Other_Ded_Desc1_AfterTax, Other_Ded_Desc2_AfterTax, Other_Ded_Desc3_AfterTax, ' +
            ' Other_Ded_Desc4_AfterTax, Other_Ded_Desc5_AfterTax, Other_Ded_Desc6_AfterTax, Other_Ded_Desc7_AfterTax, MPF_EE, MPF_ER ' +
            ' ) ' +

          'VALUES ' +
            '('+sPeriod2+', '+sPeriod2+', '+ADODataSet1.FieldByName('Employee_PIN').AsString+', '+
              ...
              CurrToStr(vBonusOthers) + ',' + CurrToStr(vBonusTraining) + ',' + CurrToStr(vBonusFreeField1) + ',''' + sRemarks +
              ''',' + CurrToStr(vMP2EmployeeShare) + ',' + CurrToStr(vMP2EmployerShare) +

              ',' + CurrToStr(vOther_EarnAfterTax1) +',' + CurrToStr(vOther_EarnAfterTax2) +
              ',' + chr(39)+vOther_EarnAfterTax_Desc1+chr(39)+', '+chr(39)+vOther_EarnAfterTax_Desc2+chr(39)+

              ',' + CurrToStr(vOther_DedAfterTax1) +',' + CurrToStr(vOther_DedAfterTax2) +
              ',' + CurrToStr(vOther_DedAfterTax3) +',' + CurrToStr(vOther_DedAfterTax4) +
              ',' + CurrToStr(vOther_DedAfterTax5) +',' + CurrToStr(vOther_DedAfterTax6) + ',' + CurrToStr(vOther_DedAfterTax7) +
              ',' + chr(39)+vOther_DedAfterTax_Desc1+chr(39)+', '+chr(39)+vOther_DedAfterTax_Desc2+chr(39)+
              ',' + chr(39)+vOther_DedAfterTax_Desc3+chr(39)+', '+chr(39)+vOther_DedAfterTax_Desc4+chr(39)+
              ',' + chr(39)+vOther_DedAfterTax_Desc5+chr(39)+', '+chr(39)+vOther_DedAfterTax_Desc6+chr(39)+ ', '+chr(39)+vOther_DedAfterTax_Desc7+chr(39)+
              ',' + CurrToStr(vMPF_EE) +',' + CurrToStr(vMPF_ER) +
...
{function TfrmPayrollProcess.GetAnnualSalary(const AEmpCode: string): Double;
var
  qry: TADOQuery;
  YearToDateBasic: Double;
...
begin
  ...
  // Since payroll is processed every 21st of the month,
  // if today is May 23, 2026,
  // current processing payroll period = 04/21/2026
  //
  // Completed payrolls:
  // 01/21/2026
  // 02/21/2026
  // 03/21/2026
  //
  // We only get completed payrolls.

  CutoffDate := EncodeDate(CurrentYear, MonthOf(Date) - 1, 21);

  qry := TADOQuery.Create(nil);
  try
    qry.Connection := ADOConnection1; // assign your connection here

    // =====================================================
    // GET JAN-MARCH TOTAL BASIC PAY FROM PAYROLL PROCESS
    // =====================================================
    qry.SQL.Clear;
    qry.SQL.Add('SELECT ISNULL(SUM(Basic_Pay1),0) AS TotalBasic ');
    qry.SQL.Add('FROM dtaPayrollProcess WITH (NOLOCK) ');
    qry.SQL.Add('WHERE Employee_PIN = :EmpCode ');
    qry.SQL.Add('AND Period1 >= :StartDate ');
    qry.SQL.Add('AND Period1 < :CutoffDate ');

    qry.Parameters.ParamByName('EmpCode').Value := AEmpCode;
    qry.Parameters.ParamByName('StartDate').Value :=
      EncodeDate(CurrentYear, 1, 21);

    qry.Parameters.ParamByName('CutoffDate').Value := CutoffDate;

    qry.Open;

    YearToDateBasic := qry.FieldByName('TotalBasic').AsFloat;

    qry.Close;

    // =====================================================
    // GET LATEST MONTHLY SALARY FROM EMPLOYEE MASTER
    // =====================================================
    qry.SQL.Clear;
    qry.SQL.Add('SELECT ISNULL(Monthly_Rate,0) AS Monthly_Rate ');
    qry.SQL.Add('FROM dtaEmployees WITH (NOLOCK) ');
    qry.SQL.Add('WHERE Employee_PIN = :EmpCode ');
    ...
    CurrentMonthlySalary :=
      qry.FieldByName('Monthly_Rate').AsFloat;
    ...
    // =====================================================
    // Example:
    // Current payroll processing = April
    // Remaining months = April to December = 9 months

    RemainingMonths := 12 - (MonthOf(Date) - 1);
    ...
    AnnualSalary :=
      YearToDateBasic +
      (CurrentMonthlySalary * RemainingMonths);
...
function TfrmPayrollProcess.GetAnnualSalary(
  const AEmpCode: string;
  const AFromDate: TDate): Double;
var
  qry: TADOQuery;
  YearToDateBasic: Double;
  CurrentMonthlySalary: Double;
...
begin
  ...
  CurrentYear := YearOf(AFromDate);

  // Previous month's 21st cutoff
  // Example:
  // AFromDate = 05/23/2026
  // CutoffDate = 04/21/2026
  CutoffDate :=
    EncodeDate(
      YearOf(IncMonth(AFromDate, -1)),
      MonthOf(IncMonth(AFromDate)),
      21
    );
  ...
  try
    ...
    // =====================================================
    // GET YTD BASIC PAY FROM PAYROLL PROCESS
    // =====================================================
    qry.SQL.Clear;
    qry.SQL.Add('SELECT ISNULL(SUM(Basic_Pay1),0) AS TotalBasic ');
    qry.SQL.Add('FROM dtaPayrollProcess WITH (NOLOCK) ');
    ...
    // Example:
    // AFromDate = May 2026
    // Current payroll processing month = April
    // Remaining months = April to December = 9
    RemainingMonths := 12 - (MonthOf(AFromDate) - 1);
...
function TfrmPayrollProcess.GetYearToMonthSalary(
  const AEmpCode: string;
  const AFromDate: TDate): Double;
var
  qry: TADOQuery;
...
begin
  ...
  try
    ...
    // =====================================================
    // TOTAL YEAR TO MONTH SALARY
    // =====================================================
    YearToMonthSalary :=
      YearToDateBasic + CurrentMonthlySalary;

    Result := YearToMonthSalary;
...
function TfrmPayrollProcess.GetYearToMonthBonus(const AEmpCode: string;
  const CurrentCutoffBonus: Double): Double;
var
  qry: TADOQuery;
  YearToDateBonus: Double;
  TotalYearToMonthBonus: Double;
  CurrentYear: Integer;
  CutoffDate: TDate;
begin
  Result := 0;
  ...
  // Example:
  // Today = May 23, 2026
  // Current payroll processing = 04/21/2026
  //
  // We only get COMPLETED payrolls:
  // 01/21/2026
  // 02/21/2026
  // 03/21/2026
  //
  // Then add current cutoff bonus manually.
  ...
  try
    ...
    // =====================================================
    // GET YEAR-TO-DATE BONUS
    // =====================================================
    qry.SQL.Clear;
    qry.SQL.Add('SELECT ISNULL(SUM(Bonus_UPH + Bonus_Quality),0) AS TotalBonus ');
    qry.SQL.Add('FROM dtaPayrollProcess WITH (NOLOCK) ');
    qry.SQL.Add('WHERE Employee_PIN = :EmpCode ');
    qry.SQL.Add('AND Period1 >= :StartDate ');
    qry.SQL.Add('AND Period1 < :CutoffDate ');
    ...
    YearToDateBonus :=
      qry.FieldByName('TotalBonus').AsFloat;

    qry.Close;

    // =====================================================
    // ADD CURRENT PAY CUTOFF BONUS
    // =====================================================
    TotalYearToMonthBonus :=
      YearToDateBonus + CurrentCutoffBonus;

    Result := TotalYearToMonthBonus;
...
procedure TfrmPayrollProcess.CalculateBonusTax(
  const AEmpPin: Integer;
  const ABasicSalary: Currency;
  const ABonusAmount: Currency;
  out AdditionalTaxableIncome: Currency;
  out BonusTax: Currency);
var
  qry: TADOQuery;
  Balance: Currency;
  MonthlyThreshold: Currency;
  MonthlyThresholdUpdated: Currency;
begin
  ...
  qry := TADOQuery.Create(nil);
  try
    qry.Connection := ADOConnection1; // Replace with your connection

    qry.SQL.Text :=
      'SELECT Balance ' +
      'FROM tempThresholdSummary ' +
      'WHERE [Employee Number] = :EmpPin';

    qry.Parameters.ParamByName('EmpPin').Value := AEmpPin;
    qry.Open;

    if not qry.Eof then
      Balance := qry.FieldByName('Balance').AsCurrency;

    // 15% of Basic Salary
    MonthlyThreshold := ABasicSalary * 0.15;

    // Default value
    MonthlyThresholdUpdated := MonthlyThreshold;

    // If balance is negative, deduct it from threshold
    if Balance < 0 then
      MonthlyThresholdUpdated := MonthlyThreshold - Abs(Balance)
    else if Balance > 0 then
      MonthlyThresholdUpdated := Balance
    else
      MonthlyThresholdUpdated := 0;

    // Prevent negative threshold
    if MonthlyThresholdUpdated < 0 then
      MonthlyThresholdUpdated := 0;

    // Compare bonus against updated threshold
    if ABonusAmount > MonthlyThresholdUpdated then
    begin
      AdditionalTaxableIncome :=
        ABonusAmount - MonthlyThresholdUpdated;

      BonusTax :=
        MonthlyThresholdUpdated * 0.05;
    end
    else
    begin
      AdditionalTaxableIncome := 0;
      BonusTax := ABonusAmount * 0.05;
    end;
