...
type
  TfrmExtractionAFLAC = class(TForm)
    GroupBox1: TGroupBox;
    Label1: TLabel;
    Edit1: TEdit;
    Button1: TButton;
...
var
  frmExtractionAFLAC: TfrmExtractionAFLAC;
  sqlserver : string;
  sqldatabase : string;
  sqluser : string;
  sqlpwd : string;
...
procedure TfrmExtractionAFLAC.Button3Click(Sender: TObject);
begin
...
procedure TfrmExtractionAFLAC.Button2Click(Sender: TObject);
begin
  frmExtractionAFLAC.close;
end;

procedure TfrmExtractionAFLAC.Button1Click(Sender: TObject);
var
...
//   ADOQuery1.Active:= True;
   ...
   repeat
      ...
      vTask_ID := trim((oSheet.Cells[Keyer,8]));
      ...
      begin
        ...
        vKeyer_ID := trim((oSheet.Cells[Keyer,4]));
        vBatches := (oSheet.Cells[Keyer,10]);
        vClaims := (oSheet.Cells[Keyer,12]);
        vPulls := (oSheet.Cells[Keyer,14]);
        vMins := (oSheet.Cells[Keyer,15]);
      end;

      if vProcessed >'' then
      begin
      //repeat
         edit2.Text := 'Extracting AFLAC Report...' + chr(39)+ vProcessed + chr(39)+ ' '+ chr(39) + (vKeyer_ID) + chr(39)+' '+ chr(39) + (vTask_ID) + chr(39)+' '+
         inttostr(vBatches) +' '+inttostr(vClaims)+' '+inttostr(vPulls)+' '+ floattostr(vMins);
         edit2.Refresh;

         ADODATASET1.Connection := ADOCONNECTION1;
         ADODATASET1.CommandText := 'SELECT * FROM dtaProduction WHERE Tran_Date = ' + chr(39)+(vProcessed)+chr(39) + ' AND Keyer_ID = '+ chr(39) + vKeyer_ID+ chr(39) + 'AND Task_ID ='+chr(39) +VTask_ID+ chr(39);
         ADODATASET1.Open;
         ...
         begin
             //ADOQUERY1.Insert;
             //ADOQUERY1.FieldByName('Date').AsString := vProcessed;
             //ADOQUERY1.FieldByName('Keyer_ID').AsString := vKeyer_ID;
             //ADOQUERY1.FieldByName('Task_ID').AsString := vTask_ID;
             //ADOQUERY1.FieldByName('Batches').AsInteger := vBatches;
             //ADOQUERY1.FieldByName('Claims').AsInteger := vClaims;
             //ADOQUERY1.FieldByName('Pulls').AsInteger := vPulls;
             //ADOQUERY1.FieldByName('Time_Taken').AsFloat := vMins;
           ...
           begin
              ...
              begin
                SQLUpdate := 'UPDATE dtaProduction ' +
                  ...
                  ' WHERE Tran_Date = '+ chr(39) + (vProcessed) + chr(39) +' AND Keyer_ID = '+ chr(39) + (vKeyer_ID) + chr(39) +' AND Task_ID = '+ chr(39) + (vTask_ID)+chr(39);
                ADOCONNECTION1.Execute(SQLupdate);
             end;
           end;
         ADODATASET1.Close;
...
//   ADOQUERY1.Active := False;
   ...
   edit2.Text := 'Extraction of '+intToStr(rec_count)+ ' ' + 'AFLAC records is Complete!';

end;

procedure TfrmExtractionAFLAC.FormCreate(Sender: TObject);
begin
   ADOConnection1.ConnectionString := CommonModule.ReadInitConn(CommonModule.ToConnect('Null'));
   ADOConnection1.Connected := TRUE;
end;

procedure TfrmExtractionAFLAC.Process_Log;
var
  ID: String;
  SQL: String;
begin
  ...
  SQL := SQL + chr(39) + 'Extraction - AFLAC' + chr(39) + ', ';
